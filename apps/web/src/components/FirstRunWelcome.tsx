...
export function FirstRunWelcome({ data, onDismiss, onGuide, onNavigate, onOpenModelCatalog }: {
  data: BootstrapData;
  onDismiss: () => void;
  onGuide: () => void;
  onNavigate: (route: AppRoute) => void;
  /** Non-admin step 2: see which models exist and request access. */
  ...
  return (
    <section className="first-run-welcome" aria-labelledby="first-run-title">
      ...
      <div className="first-run-actions">
        {(owner || admin) && <button className="primary-button" type="button" onClick={() => onNavigate(setupRoute)}><Settings2 size={16} />{owner && !ready ? "Set up models" : "Manage access"}<ArrowRight size={15} /></button>}
