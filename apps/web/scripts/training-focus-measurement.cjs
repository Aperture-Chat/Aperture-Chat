...
const retention = {
  retentionNavigation: first("[role=tab]:text-is(\"Audit\")"),
  retentionWorkspace: first(".retention-view-switch"),
  retentionPanel: union(".retention-schedule-fields > label:first-child, .retention-duration-slider, .retention-preset-labels"),
  retentionSources: union(".retention-source-section > h4, .retention-source-section > p:not(.retention-explanation), .retention-source-section > .retention-field-grid, .retention-source-section > label:not(.retention-checkbox), .retention-source-section > button"),
  retentionSensitive: first(".retention-source-section .retention-checkbox"),
  retentionHolds: first(".retention-tag-management details"),
  retentionSchedulePreview: union(".retention-impact, .retention-governance > .retention-actions"),
  retentionToggles: union(`${panel("Data Retention")} .policy-toggle-row`),
  retentionTagsSwitch: first(".retention-view-switch"),
  retentionTagsExplorer: first(".retention-tag-management"),
...
const FOCUS_TARGETS = {
  user: {
    ...
    securitySignIn: first(".auth-panel"),
    securityPassword: first(".account-password-card"),
    profileEditor: first(".account-profile-form"),
    appearanceControl: first(".help-row"),
    composerShortcuts: first("[role=\"dialog\"][aria-label=\"Send options\"]"),
    ...
    modelFavorites: first(".workspace-header .model-menu"),
    modelAccessOverview: first("[role='dialog'][aria-label='Models in your organization']"),
    modelAccessRequest: first("[role='dialog'][aria-label='Models in your organization'] [data-model-id]:has(button:text-is('Request access'):not([disabled]))"),
    modelAccessPending: first("[role='dialog'][aria-label='Models in your organization'] [data-model-id]:has(button:text-is('Withdraw request'))"),
    themeSchedule: first(".theme-schedule-modal"),
    ...
    sendStreaming: first(".send-menu-stream"),
    ...
    searchCommands: first(".command-palette-panel"),
    searchRecent: first(".command-palette-panel"),
    draftSaveState: first(".document-editor-topbar"),
    draftHistory: first(".draft-history-panel"),
    draftHistoryPreview: first(".draft-history-preview"),
    unsyncedWork: first("[role='dialog'][aria-label='Only on this device']"),
    draftSettings: first(".draft-settings-panel"),
    ...
    draftAiEdit: first(".ai-composer"),
    draftSlashMenu: first(".document-slash-menu"),
    draftStatusBar: first(".document-status-bar"),
    ...
    automationsTabs: first(".agent-workspace-shell [role='group'][aria-label='Agent workspace sections']"),
    ...
    sidebarFolders: union(".chat-library-heading, .chat-section.folder-section"),
    sidebarPinned: first(".chat-section:has(#sidebar-pinned-label)"),
    sidebarPreview: first(".chat-hover-preview"),
    sidebarRowActions: union(".chat-row.has-open-menu .chat-row-more, .chat-row-menu"),
    sidebarChatsHidden: first(".chat-library-heading"),
    sidebarNavigation: first(".primary-nav"),
  ...
  admin: {
    ...
    maRequests: first(".model-access-requests-panel"),
    maRequestGroup: first(".model-access-request-actions"),
    maUserTrace: first("[role='dialog'][aria-label^='Model access trace for ']"),
    ...
    responseOverview: first(`${panel("Chat output actions")} > .panel-header`),
    responseCreate: first(`${panel("Chat output actions")} button:has-text('New response action')`),
    connResponseActions: first(`${panel("Chat output actions")} > .panel-header`),
  ...
  owner: {
    searchIndex: first(".search-index-card"),
    modelBrowsingPolicy: first(".policy-toggle-row:has([role='switch'][aria-label='Users can browse the model catalog'])"),
    ...
    providerStats: first(".provider-summary dl:has(dt:text-is('Catalog'))"),
    ...
    brandActions: union(".branding-actions, .platform-branding-panel button:text-is('Apply branding')"),
    ...
    sharedConnectors: first(`${panel("Connectors")} .connector-config-block`),
    ...
    trailRows: first("[aria-label=\"Export audit trail CSV\"]"),
